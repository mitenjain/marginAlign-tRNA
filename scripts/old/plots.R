...
if ( length(data$MappedReadLengths) > 1 && length(data$UnmappedReadLengths) > 1) {
    ...
    # density plot for identity
    xmax <- 0
    ymax <- 0
    b <- 0
    b <- max(b, nclass.FD(data$ReadIdentity[!is.na(data$ReadIdentity)]))

    hists <- hist(data$ReadIdentity[!is.na(data$ReadIdentity)], plot=F, breaks=b)
    xmax <- max(xmax, hists$mids)
    ymax <- max(ymax, hists$counts)

    #whatever
    plot(1, xlim=c(0,xmax), ylim=c(0,ymax), main=paste("Average % Identity", args[1], sep="\n"), xlab="Identity", type="n", ylab="Frequency")


    q <- vector()
    x <- hists
    points(x$mids, x$counts, col="magenta", pch=15, cex=0.4)
    lines(x$mids, x$counts, col="magenta", pch=15)
    q <- paste(formatC(round(100*mean(data$ReadIdentity[!is.na(data$ReadIdentity)]),1),format="f",digits=1), sep= "")

    legend(x="top", col="magenta", pch=15, legend=q, ncol=2, title="Average % Identity", cex=0.55,bty="n")
    ...
    plot(as.numeric(mean(data$InsertionsPerBase)), as.numeric(mean(data$DeletionsPerBase)), xlab="Avg. Insertions Per Aligned Read Base", ylab="Avg. Deletions Per Aligned Read Base", main=args[1], col="magenta", pch=15, xlim=c(0,0.2), ylim=c(0,0.2), cex.main=0.9)
    
    #scatterplot of Avg identity coverage vs Avg # of indels per base
    plot(100 * as.numeric(mean(data$ReadIdentity)), as.numeric(mean(data$InsertionsPerBase)) + as.numeric(mean(data$DeletionsPerBase)), ylab="Avg. Indels Per Aligned Read Base", xlab="Avg. Match Identity", main=args[1], col="magenta", pch=15, xlim=c(0,100), cex.main=0.9)
